...
int uniques_main(int argc, char *argv[]){
    ...
    int c;
    while ((c = getopt(argc, argv, "al:w:")) >= 0) {
    ...
    if ( optind == argc || argc > optind + 1) {
        fprintf(stderr, "\nUsage: amplicon-tk uniques [options] <in.fa/q>\n\n");
        fprintf(stderr, "Options:\n");
        fprintf(stderr, "  -a      print size annotation to stderr.\n");
    ...
    if(opt.annotation) fprintf(stderr, "#unique\tsize\n");
    for (i = 0; i < N; ++i){
        k = khint[i];
        printf(">%s%d", opt.lablel, i + 1);
        if(opt.annotation) fprintf(stderr, "%s%d\t%d\n", opt.lablel, i + 1, kh_val(h, k));
